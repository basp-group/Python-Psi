...
class optparam:
    ...
    def __init__(self, positivity=True, nu1=1., nu2=1., gamma=1.e-3, tau=0.49, rel_obj=1.e-3, max_iter=200, 
                 ...
                 adapt_eps=False, adapt_eps_begin=50, adapt_eps_rel_obj=1.e-3, adapt_eps_step=100, adapt_eps_tol_in=0.99, adapt_eps_tol_out=1.01, adapt_eps_change_percentage=0.5*(np.sqrt(5)-1)):
...
#         self.sol_steps = [inf]  # saves images at the given iterations
        ...
        self.adapt_eps_change_percentage = adapt_eps_change_percentage # the weight of the update w.r.t the l2 norm of the residual data
...
def forward_backward_primal_dual(y, A, At, G, Gt, mask_G, SARA, epsilon, epsilons, param):
    ...
    for it in np.arange(param.max_iter):
        ...
        ## Adjust the l2 bound ##
        if adapt_eps and it + 1 >= param.adapt_eps_begin and rel_error < param.adapt_eps_rel_obj and it + 1 >= eps_it + param.adapt_eps_step:
        ...
        if rel_error < param.rel_obj and (norm2 <= param.adapt_eps_tol_out * epsilon) and (norm2 >= param.adapt_eps_tol_in * epsilon):
            flag = 1
            break
