        ...
        (param.use_reweight_eps and norm2 <= epsilons and param.reweight_min_steps_rel_obj < it  - reweight_last_step_iter and rel_error < param.reweight_rel_obj):
            ...
            for k in np.arange(P): 
                if SARA.basis[k] == 'self': 
                    weights[k] = param.reweight_alpha / (param.reweight_alpha + abs(SARA.Psit[k](xsol)/np.sqrt(P)))
                else:              
                    weights[k] = param.reweight_alpha / (param.reweight_alpha + abs(SARA.coef2vec(SARA.Psit[k](xsol))/np.sqrt(P)))
    
            reweight_alpha = param.reweight_alpha_ff * param.reweight_alpha
